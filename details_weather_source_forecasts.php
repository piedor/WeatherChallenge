<?php
    ...
    // Recupera la previsione della sorgente meteo
    ...
    // Recupera il nome della sorgente meteo
    $query = "SELECT name FROM weather_sources WHERE id = ?";
    $stmt = $__con->prepare($query);
    $stmt->bind_param("i", $forecast['weather_source_id']);
    $stmt->execute();
    $result = $stmt->get_result();
    $fullNameForecaster = $result->fetch_assoc()["name"];
    $stmt->close();

    // Recupera i dati meteo reali salvati
    ...
    // Preparazione dati per il grafico orario
    $hoursSeries = [];
    for ($i = 0; $i < 24; $i++) {
...
<html lang="it">
    <head>
        ...
        <style>
            ...
            .table-modern thead { background-color: #f8f9fa; text-transform: uppercase; font-size: 0.75rem; }
    ...
    <body class="bg-light">
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    <div class="text-center mb-4">
                        <h3 class="fw-light">Previsione di <span class="fw-bold"><?= htmlspecialchars($fullNameForecaster) ?></span></h3>
                        <div class="display-4 text-success fw-bold"><?= htmlspecialchars($forecast['accuracy']) ?>%</div>
                        <p class="text-muted">Accuratezza Calcolata</p>
                    </div>
                    ...
                    <div class="d-grid gap-2">
                        <button class="btn btn-outline-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapseMath">
                            Dettagli Analisi 🔍
                        </button>
                    </div>
                    ...
                    <div class="collapse mt-4" id="collapseMath">
                        ...
                        <div class="mt-4">
                            <h6 class="text-uppercase fw-bold text-muted small">Logica di Calcolo</h6>
                    ...
                    <div class="mt-4 pt-3 border-top">
                        <a href="weather_source_forecast_details.php?id=<?= urlencode($forecast['weather_source_id']) ?>" class="btn btn-secondary w-100">
                            ← Torna alle previsioni della sorgente
                        </a>
                    </div>
                </div>
            </div>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                // Applica localizzazione italiana da setup.js
                setupCharts();
                ...
                // Usa la funzione centralizzata definita in setup.js
